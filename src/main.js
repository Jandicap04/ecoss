...
const seed = {
  ...
  subscription: { status: 'trialing', daysLeft: 5, renews: '28 sep 2026' },
  quotes: [
    { id: 'CR-1048', customer: 'Laura Gómez', vehicle: 'Renault Duster 2021', amount: 780000, status: 'approved', date: 'Hoy, 09:42', items: ['Cambio de aceite', 'Alineación y balanceo'] },
    { id: 'CR-1047', customer: 'Carlos Mendoza', vehicle: 'Mazda 3 2019', amount: 420000, status: 'pending', date: 'Ayer, 16:18', items: ['Revisión de frenos'] },
    { id: 'CR-1046', customer: 'Andrés Ruiz', vehicle: 'Chevrolet Onix 2022', amount: 295000, status: 'sent', date: 'Ayer, 11:06', items: ['Cambio de batería'] },
    { id: 'CR-1045', customer: 'Diana Castro', vehicle: 'Kia Picanto 2020', amount: 960000, status: 'approved', date: '12 sep, 14:20', items: ['Kit de distribución'] },
    { id: 'CR-1044', customer: 'Felipe Torres', vehicle: 'Toyota Corolla 2018', amount: 180000, status: 'rejected', date: '11 sep, 10:03', items: ['Diagnóstico general'] },
  ],
  customers: [
...
function loadState() {
  try {
    ...
    return stored ? { ...seed, ...stored, profile: { ...seed.profile, ...stored.profile }, subscription: { ...seed.subscription, ...stored.subscription } } : structuredClone(seed)
  ...
  document.querySelector('#app').innerHTML = `
    <div class="app-shell">
      <aside class="sidebar" id="sidebar">
        ...
        <div class="workspace-switcher"><span class="avatar avatar-small">${escapeHtml(state.profile.initials)}</span><span><b>${escapeHtml(state.profile.workshop)}</b><small>Plan prueba</small></span><span class="chevron">⌄</span></div>
...
function viewTitle() { return ({ dashboard: 'Resumen', quotes: 'Cotizaciones', customers: 'Clientes', services: 'Servicios', settings: 'Ajustes' })[activeView] }
function trialBanner() { return state.subscription.status === 'trialing' ? `<div class="trial-banner"><div class="trial-message"><span class="trial-spark">✦</span><span><b>Tu prueba gratuita termina en ${state.subscription.daysLeft} días.</b> Sigue cotizando sin interrupciones.</span></div><button class="text-button" data-action="upgrade">Ver planes <span>${icons.arrow}</span></button></div>` : state.subscription.status === 'canceled' ? `<div class="trial-banner cancel-banner"><div class="trial-message"><span class="trial-spark">!</span><span><b>Tu suscripción termina el ${state.subscription.renews}.</b> Tus datos seguirán disponibles.</span></div><button class="text-button" data-action="upgrade">Reactivar plan <span>${icons.arrow}</span></button></div>` : '' }
...
function handleAction(event) {
  ...
  if (action === 'upgrade') modal = { type: 'upgrade' }
  if (action === 'quote-detail') modal = { type: 'detail', id: event.currentTarget.dataset.id }
  if (action === 'close-modal') modal = null
  if (action === 'activate-plan') { state.subscription = { status: 'active', daysLeft: 30, renews: '23 oct 2026' }; saveState(); modal = null; showToast('Plan profesional activado', 'success') }
...
function submitQuote(event) { event.preventDefault(); const data = new FormData(event.currentTarget); const prices = [...event.currentTarget.querySelectorAll('.line-price')].map((input) => Number(input.value) || 0); const amount = prices.reduce((sum, value) => sum + value, 0); if (!amount) { showToast('Agrega al menos un servicio con precio', 'error'); return } state.quotes.unshift({ id: `CR-${1050 + state.quotes.length}`, customer: data.get('customer'), vehicle: data.get('vehicle'), amount, status: 'draft', date: 'Ahora', items: [...event.currentTarget.querySelectorAll('.line-price')].map((input) => input.closest('.quote-line').querySelector('select').selectedOptions[0]?.textContent.split(' · ')[0] || 'Servicio personalizado') }); saveState(); modal = null; showToast('Cotización guardada como borrador', 'success'); render() }
